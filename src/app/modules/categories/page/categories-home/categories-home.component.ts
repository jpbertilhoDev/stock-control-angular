  ...
  handleDeleteCategoryAction(event: DeleteCategoryAction): void {
    if (event) {
      this.confirmationService.confirm({
        message: `Você deseja remover a categoria: ${event.categoryName}`,
        ...
        .subscribe({
          next: (response) => {
            this.messageService.add({
              severity: 'success',
              summary: 'Sucesso',
              detail: `Categoria ${category_id} removido com sucesso!`,
              life: 2500,
            });
            this.getAllCategories();
          },
          error: (err) => {
            ...
            this.messageService.add({
              severity: 'error',
              summary: `Erro ao remover a categoria: ${category_id}`,
              life: 3000,
            });
          },
        });
